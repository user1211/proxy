...
function bindQueue(connection) {
  ...
  connection.queue.on("error", error => {
    killConnection(connection);
    log("bot connection error", error.message);
  });
  connection.queue.on("message", function(message) {
    log("\nmessage from bot to burgerjoint:\n", message);
    let data = null;
    try {
      data = JSON.parse(message);
    } catch (e) {
      return log("\ncan't parse message as JSON from bot:\n", message);
    }
    switch (data.type) {
      case "auth": {
...
function sendToMiner(connection, payload) {
  ...
  if (connection.online) {
    ...
    log(
      "\nfailed to send message to bot cos it was offline:",
      coinHiveMessage
    );
  }
}
...
function connectSocket(connection, port, host) {
  ...
  connection.socket.connect(+port, host, function() {
    ...
    connection.socket.on("data", function(buffer) {
      ...
      log("\nmessage from burgerjoint to bot:\n", stratumMessage);
      let data = null;
      try {
    ...
    connection.socket.on("close", function() {
      log("connection to burgerjoint closed");
      killConnection(connection);
    });
    connection.socket.on("error", function(error) {
      log(
        "burgerjoint connection error",
